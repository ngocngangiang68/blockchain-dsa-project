...
def generate_mock_transactions(n=10000, base_timestamp=None):
    """Hàm tạo ra danh sách 10.000 giao dịch mẫu để đưa vào Mempool.
    ...
    for _ in range(n):
        ...
        # ✅ THAY ĐỔI Ở ĐÂY: Thay vì random.uniform, dùng random.randint
        # Tạo timestamp trong khoảng 24 giờ (86400 giây) nhưng chỉ số nguyên
        # Điều này vẫn giữ tính ngẫu nhiên nhưng CỐ ĐỊNH khi có seed
        random_offset = random.randint(0, 86400)  # ✅ THAY ĐỔI: uniform → randint
        random_timestamp = base_timestamp + random_offset  # ✅ THAY ĐỔI: trừ (-) → cộng (+)
